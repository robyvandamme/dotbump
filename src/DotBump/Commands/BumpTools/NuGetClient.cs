...
internal class NuGetClient(ILogger logger) : INuGetClient, IDisposable
{
    private static readonly JsonSerializerOptions s_jsonSerializerOptions =
        new() { PropertyNameCaseInsensitive = false, }; // TODO: false is the default so the option can be removed.
    ...
    {
        logger.MethodStart(nameof(NuGetClient), nameof(GetServiceIndexesAsync), sources);
        ...
        logger.MethodReturn(nameof(NuGetClient), nameof(GetServiceIndexesAsync), list);
    ...
    public async Task<RegistrationIndex?> GetPackageInformationAsync(
        ...
        string packageId)
    {
        logger.MethodStart(nameof(NuGetClient), nameof(GetServiceIndexesAsync), baseUrls, packageId);
        ...
        foreach (var url in baseUrls)
        {
            ...
            if (registrationIndex != null)
            {
                ...
                logger.MethodReturn(nameof(NuGetClient), nameof(GetServiceIndexesAsync), registrationIndex);
                return registrationIndex;
            }
        }

        logger.Debug("No service indexes found for package {PackageId} at {BaseUrls}", packageId, baseUrls);
        logger.MethodReturn(nameof(NuGetClient), nameof(GetServiceIndexesAsync));
    ...
    {
        logger.MethodStart(nameof(NuGetClient), nameof(GetRelevantDetailCatalogPagesAsync), catalogPages);
        ...
        logger.MethodReturn(nameof(NuGetClient), nameof(GetRelevantDetailCatalogPagesAsync), result);
